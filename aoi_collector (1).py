...
class CollectorWorker(QThread):
    ...
    def _collect(self):
        ...
        for f in walk:
            ...
            nl = f.name.lower()
            is_pass = PASS_KEYWORD in nl
            is_fail = FAIL_KEYWORD in nl
            # Chi thu thap file co chua ALL PASS hoac fail
            if self.filter_mode == "both":
                if not is_pass and not is_fail: continue
            elif self.filter_mode == "pass":
                if not is_pass: continue
            elif self.filter_mode == "fail":
...
class AOICollector(QMainWindow):
    ...
    def _build(self):
        ...
        self.filter_combo.addItems(["Both  (ALL PASS + fail)",
    ...
    def _start(self):
        ...
        self.worker.filter_mode = ["both","pass","fail"][self.filter_combo.currentIndex()]
    ...
    def _on_done(self,s):
        ...
        for l in [f"{'='*52}",
                  ...
                  f"  PASS : {s['pass']}    FAIL : {s['fail']}    Errors : {s['errors']}",
                  f"  ZIP  : {s['zip_path']}",
